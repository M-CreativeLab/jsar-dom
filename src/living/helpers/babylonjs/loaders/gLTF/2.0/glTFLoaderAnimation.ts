...
export function getVector3(_target: any, source: Float32Array, offset: number, scale: number): BABYLON.Vector3 {
  return BABYLON.Vector3.FromArray(source, offset).scaleInPlace(scale);
}

/** @internal */
export function getQuaternion(_target: any, source: Float32Array, offset: number, scale: number): BABYLON.Quaternion {
  return BABYLON.Quaternion.FromArray(source, offset).scaleInPlace(scale);
}
...
export abstract class AnimationPropertyInfo {
  ...
  protected _buildAnimation(name: string, fps: number, keys: any[]): BABYLON.Animation {
    const babylonAnimation = new BABYLON.Animation(name, this.name, fps, this.type);
    babylonAnimation.setKeys(keys);
    return babylonAnimation;
  }

  /** @internal */
  public abstract buildAnimations(target: any, name: string, fps: number, keys: any[], callback: (babylonAnimatable: BABYLON.IAnimatable, babylonAnimation: BABYLON.Animation) => void): void;
}

/** @internal */
export class TransformNodeAnimationPropertyInfo extends AnimationPropertyInfo {
  /** @internal */
  public buildAnimations(target: INode, name: string, fps: number, keys: any[], callback: (babylonAnimatable: BABYLON.IAnimatable, babylonAnimation: BABYLON.Animation) => void): void {
  ...
  public buildAnimations(target: INode, name: string, fps: number, keys: any[], callback: (babylonAnimatable: BABYLON.IAnimatable, babylonAnimation: BABYLON.Animation) => void): void {
    if (target._numMorphTargets) {
      for (let targetIndex = 0; targetIndex < target._numMorphTargets; targetIndex++) {
        const babylonAnimation = new BABYLON.Animation(`${name}_${targetIndex}`, this.name, fps, this.type);
...
export const nodeAnimationData = {
  translation: [new TransformNodeAnimationPropertyInfo(BABYLON.Animation.ANIMATIONTYPE_VECTOR3, 'position', getVector3, () => 3)],
  rotation: [new TransformNodeAnimationPropertyInfo(BABYLON.Animation.ANIMATIONTYPE_QUATERNION, 'rotationQuaternion', getQuaternion, () => 4)],
  scale: [new TransformNodeAnimationPropertyInfo(BABYLON.Animation.ANIMATIONTYPE_VECTOR3, 'scaling', getVector3, () => 3)],
  weights: [new WeightAnimationPropertyInfo(BABYLON.Animation.ANIMATIONTYPE_FLOAT, 'influence', getWeights, (target) => target._numMorphTargets!)],
};
