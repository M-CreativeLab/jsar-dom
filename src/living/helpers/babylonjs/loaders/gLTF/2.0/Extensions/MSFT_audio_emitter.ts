...
type AnimationEvent = BABYLON.AnimationEvent;
const { Vector3, AnimationEvent, Tools } = BABYLON;
...
interface ILoaderEmitter extends IMSFTAudioEmitter_Emitter, IArrayItem {
  _babylonData?: {
    sound?: BABYLON.WeightedSound;
    loaded: Promise<void>;
  };
  _babylonSounds: BABYLON.Sound[];
}
  ...
  private _loadEmitterAsync(context: string, emitter: ILoaderEmitter): Promise<void> {
    ...
    if (!emitter._babylonData) {
      ...
      for (let i = 0; i < emitter.clips.length; i++) {
        ...
        clipPromises.push(
          this._loadClipAsync(`${clipContext}/${emitter.clips[i].clip}`, clip).then((objectURL: string) => {
            const sound = (emitter._babylonSounds[i] = new BABYLON.Sound(name, objectURL, this._loader.babylonScene, null, options));
      ...
      const promise = Promise.all(clipPromises).then(() => {
        ...
        const weightedSound = new BABYLON.WeightedSound(emitter.loop || false, emitter._babylonSounds, weights);
