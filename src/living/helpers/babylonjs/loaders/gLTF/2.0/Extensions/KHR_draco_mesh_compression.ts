...
export class KHR_draco_mesh_compression implements IGLTFLoaderExtension {
   ...
   */
  public dracoCompression?: BABYLON.DracoCompression;
  ...
  constructor(loader: GLTFLoader) {
    this._loader = loader;
    this.enabled = BABYLON.DracoCompression.DecoderAvailable && this._loader.isExtensionUsed(NAME);
  }
    ...
    return GLTFLoader.LoadExtensionAsync<IKHRDracoMeshCompression, Geometry>(context, primitive, this.name, (extensionContext, extension) => {
      ...
      if (!bufferView._dracoBabylonGeometry) {
        bufferView._dracoBabylonGeometry = this._loader.loadBufferViewAsync(`/bufferViews/${bufferView.index}`, bufferView).then((data) => {
          const dracoCompression = this.dracoCompression || BABYLON.DracoCompression.Default;
          return dracoCompression
            // TODO .decodeMeshAsync(data, attributes, dividers)
