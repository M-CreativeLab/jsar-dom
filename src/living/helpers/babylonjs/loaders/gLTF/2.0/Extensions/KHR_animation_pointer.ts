...
export class KHR_animation_pointer implements IGLTFLoaderExtension {
  ...
  public _loadAnimationChannelAsync(
    ...
    onLoad: (babylonAnimatable: BABYLON.IAnimatable, babylonAnimation: Animation) => void
    ...
    if (channel.target.path !== AnimationChannelTargetPath.POINTER) {
      BABYLON.Logger.Warn(`${context}/target/path: Value (${channel.target.path}) must be (${AnimationChannelTargetPath.POINTER}) when using the ${this.name} extension`);
    }

    if (channel.target.node != undefined) {
      BABYLON.Logger.Warn(`${context}/target/node: Value (${channel.target.node}) must not be present when using the ${this.name} extension`);
    }
    ...
    if (!targetInfo) {
      BABYLON.Logger.Warn(`${extensionContext}/pointer: Invalid pointer (${pointer}) skipped`);
      return null;
    }
  ...
  private _parseAnimationPointer(context: string, pointer: string): Nullable<IAnimationTargetInfo> {
    if (!pointer.startsWith('/')) {
      BABYLON.Logger.Warn(`${context}: Value (${pointer}) must start with a slash`);
      return null;
    }
