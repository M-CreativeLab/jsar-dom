...
interface IBufferInfo {
  ...
  loaded: BABYLON.Deferred<ArrayBufferView>;
}
...
export class MSFT_lod implements IGLTFLoaderExtension {
   ...
   */
  public onNodeLODsLoadedObservable = new BABYLON.Observable<number>();
   ...
   * Dispose the loader to cancel the loading of the next level of LODs.
   */
  public onMaterialLODsLoadedObservable = new BABYLON.Observable<number>();
  ...
  private _nodeSignalLODs = new Array<BABYLON.Deferred<void>>();
  ...
  private _materialSignalLODs = new Array<BABYLON.Deferred<void>>();
    ...
    return GLTFLoader.LoadExtensionAsync<IMSFTLOD, TransformNode>(context, node, this.name, (extensionContext, extension) => {
      ...
      for (let indexLOD = 0; indexLOD < nodeLODs.length; indexLOD++) {
        ...
        if (indexLOD !== 0) {
          ...
          this._nodeSignalLODs[indexLOD] = this._nodeSignalLODs[indexLOD] || new BABYLON.Deferred();
        }
  ...
  public _loadUriAsync(context: string, property: IProperty, uri: string): Nullable<Promise<ArrayBufferView>> {
    ...
    if (this._nodeIndexLOD !== null) {
      ...
      this._nodeSignalLODs[previousIndexLOD] = this._nodeSignalLODs[previousIndexLOD] || new BABYLON.Deferred<void>();
      ...
      this._materialSignalLODs[previousIndexLOD] = this._materialSignalLODs[previousIndexLOD] || new BABYLON.Deferred<void>();
    ...
    if (this._loader.parent.useRangeRequests && !buffer.uri) {
      ...
      const loadAsync = (bufferLODs: Array<IBufferInfo>, indexLOD: number) => {
        ...
        if (bufferLOD) {
          ...
          bufferLOD = { start: start, end: end, loaded: new BABYLON.Deferred() };
