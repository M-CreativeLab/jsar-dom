import type {
  IEXTLightsImageBased_LightReferenceImageBased,
  IEXTLightsImageBased_LightImageBased,
  IEXTLightsImageBased
...
export class EXT_lights_image_based implements IGLTFLoaderExtension {
   ...
   * @internal
   */
  public loadSceneAsync(context: string, scene: IScene): BABYLON.Nullable<Promise<void>> {
  ...
  private _loadLightAsync(context: string, light: IEXTLightsImageBased_LightImageBased): Promise<BABYLON.BaseTexture> {
    if (!light._loaded) {
      const promises = new Array<Promise<any>>();
      ...
      light._loaded = Promise.all(promises).then(() => {
        const babylonTexture = new BABYLON.RawCubeTexture(this._loader.babylonScene, null, light.specularImageSize);
        ...
        if (light.rotation) {
          let rotation = BABYLON.Quaternion.FromArray(light.rotation);
        ...
        const sphericalHarmonics = BABYLON.SphericalHarmonics.FromArray(light.irradianceCoefficients);
        sphericalHarmonics.scaleInPlace(light.intensity);

        sphericalHarmonics.convertIrradianceToLambertianRadiance();
        const sphericalPolynomial = BABYLON.SphericalPolynomial.FromHarmonics(sphericalHarmonics);

        // Compute the lod generation scale to fit exactly to the number of levels available.
        const lodGenerationScale = (imageData.length - 1) / BABYLON.Scalar.Log2(light.specularImageSize);
