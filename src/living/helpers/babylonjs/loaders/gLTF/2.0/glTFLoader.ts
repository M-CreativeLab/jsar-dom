...
export class GLTFLoader implements IGLTFLoader {
  ...
  /** @internal */
  public _assetContainer: BABYLON.Nullable<BABYLON.AssetContainer> = null;

  /** Storage */
  public _babylonLights: BABYLON.Light[] = [];
  ...
  private _rootUrl: BABYLON.Nullable<string> = null;
  private _fileName: BABYLON.Nullable<string> = null;
  private _uniqueRootUrl: BABYLON.Nullable<string> = null;
  private _gltf: IGLTF;
  private _bin: BABYLON.Nullable<BABYLON.IDataBuffer> = null;
  private _babylonScene: BABYLON.Scene;
  private _rootBabylonMesh: BABYLON.Nullable<BABYLON.Mesh> = null;
  private _defaultBabylonMaterialData: { [drawMode: number]: BABYLON.Material } = {};
  ...
  public static RegisterExtension(name: string, factory: (loader: GLTFLoader) => IGLTFLoaderExtension): void {
    if (GLTFLoader.UnregisterExtension(name)) {
      BABYLON.Logger.Warn(`Extension with the name '${name}' already exists`);
    }
   ...
   */
  public get bin(): BABYLON.Nullable<BABYLON.IDataBuffer> {
    return this._bin;
  }
   ...
   */
  public get rootBabylonMesh(): BABYLON.Nullable<BABYLON.Mesh> {
  ...
  public dispose(): void {
    ...
    (this._gltf as BABYLON.Nullable<IGLTF>) = null; // TODO
    this._bin = null;
    (this._babylonScene as BABYLON.Nullable<BABYLON.Scene>) = null; // TODO
  ...
  public importMeshAsync(
    meshesNames: any,
    scene: BABYLON.Scene,
    container: BABYLON.Nullable<BABYLON.AssetContainer>,
    ...
    return Promise.resolve().then(() => {
      ...
      let nodes: BABYLON.Nullable<Array<number>> = null;
  ...
  private _loadAsync<T>(
    rootUrl: string,
    fileName: string,
    nodes: BABYLON.Nullable<Array<number>>,
    resultFunc: () => T
  ): Promise<T> {
      ...
      .then(() => {
        ...
        if (!this.parent.skipMaterials && this.parent.loadAllMaterials && this._gltf.materials) {
          for (let m = 0; m < this._gltf.materials.length; ++m) {
            ...
            const babylonDrawMode = BABYLON.Material.TriangleFillMode;
    ...
    if (data.bin) {
      ...
      if (buffers && buffers[0] && !buffers[0].uri) {
        ...
        if (binaryBuffer.byteLength < data.bin.byteLength - 3 || binaryBuffer.byteLength > data.bin.byteLength) {
          BABYLON.Logger.Warn(`Binary buffer length (${binaryBuffer.byteLength}) from JSON does not match chunk length (${data.bin.byteLength})`);
        }

        this._bin = data.bin;
      } else {
        BABYLON.Logger.Warn('Unexpected BIN chunk');
      }
    }
  }
  ...
  private _loadExtensions(): void {
    for (const name in GLTFLoader._RegisteredExtensions) {
      ...
      if (extension.name !== name) {
        BABYLON.Logger.Warn(`The name of the glTF loader extension instance does not match the registered name: ${extension.name} !== ${name}`);
      }
  ...
  private _forEachPrimitive(node: INode, callback: (babylonMesh: BABYLON.AbstractMesh) => void): void {
  ...
  private _getGeometries(): BABYLON.Geometry[] {
    const geometries = new Array<BABYLON.Geometry>();

    const nodes = this._gltf.nodes;
    if (nodes) {
      for (const node of nodes) {
        this._forEachPrimitive(node, (babylonMesh) => {
          const geometry = (babylonMesh as BABYLON.Mesh).geometry;
  ...
  private _getMeshes(): BABYLON.AbstractMesh[] {
    const meshes = new Array<BABYLON.AbstractMesh>();
  ...
  private _getTransformNodes(): BABYLON.TransformNode[] {
    const transformNodes = new Array<BABYLON.TransformNode>();
  ...
  private _getSkeletons(): BABYLON.Skeleton[] {
    const skeletons = new Array<BABYLON.Skeleton>();
  ...
  private _getAnimationGroups(): BABYLON.AnimationGroup[] {
    const animationGroups = new Array<BABYLON.AnimationGroup>();
  ...
  private _startAnimations(): void {
    switch (this._parent.animationStartMode) {
      ...
      default: {
        BABYLON.Logger.Error(`Invalid animation start mode (${this._parent.animationStartMode})`);
        return;
      }
    }
  }
  ...
  public loadNodeAsync(
    context: string,
    node: INode,
    assign: (babylonTransformNode: BABYLON.TransformNode) => void = () => { }
    ...
    if (node.mesh == undefined || node.skin != undefined) {
      ...
      this._babylonScene._blockEntityCollection = !!this._assetContainer;
      const transformNode = new BABYLON.TransformNode(nodeName, this._babylonScene);
    ...
    return Promise.all(promises).then(() => {
      this._forEachPrimitive(node, (babylonMesh) => {
        if ((babylonMesh as BABYLON.Mesh).geometry && (babylonMesh as BABYLON.Mesh).geometry!.useBoundingInfoFromGeometry) {
  ...
  private _loadMeshAsync(
    ...
    if (primitives.length === 1) {
      ...
      this._babylonScene._blockEntityCollection = !!this._assetContainer;
      node._babylonTransformNode = new BABYLON.TransformNode(name, this._babylonScene);
  ...
  public _loadMeshPrimitiveAsync(
    ...
    if (shouldInstance && primitive._instanceData) {
      this._babylonScene._blockEntityCollection = !!this._assetContainer;
      babylonAbstractMesh = primitive._instanceData.babylonSourceMesh.createInstance(name) as BABYLON.InstancedMesh;
      ...
      this._babylonScene._blockEntityCollection = !!this._assetContainer;
      const babylonMesh = new BABYLON.Mesh(name, this._babylonScene);
      babylonMesh._parentContainer = this._assetContainer;
      this._babylonScene._blockEntityCollection = false;
      babylonMesh.overrideMaterialSideOrientation = this._babylonScene.useRightHandedSystem ?
  ...
  private _loadVertexDataAsync(
    ...
    const babylonGeometry = new BABYLON.Geometry(babylonMesh.name, this._babylonScene);
    ...
    const loadAttribute = (attribute: string, kind: string, callback?: (accessor: IAccessor) => void) => {
      ...
      promises.push(
        this._loadVertexAccessorAsync(`/accessors/${accessor.index}`, accessor, kind).then((babylonVertexBuffer) => {
          if (babylonVertexBuffer.getKind() === BABYLON.VertexBuffer.PositionKind && !this.parent.alwaysComputeBoundingBox && !babylonMesh.skeleton) {
            if (accessor.min && accessor.max) {
              const min = BABYLON.TmpVectors.Vector3[0].copyFromFloats(...(accessor.min as [number, number, number]));
              const max = BABYLON.TmpVectors.Vector3[1].copyFromFloats(...(accessor.max as [number, number, number]));
              ...
              babylonGeometry._boundingInfo = new BABYLON.BoundingInfo(min, max);
      ...
      if (kind == BABYLON.VertexBuffer.MatricesIndicesExtraKind) {
    ...
    loadAttribute('TEXCOORD_2', BABYLON.VertexBuffer.UV3Kind);
    loadAttribute('TEXCOORD_3', BABYLON.VertexBuffer.UV4Kind);
    loadAttribute('TEXCOORD_4', BABYLON.VertexBuffer.UV5Kind);
    loadAttribute('TEXCOORD_5', BABYLON.VertexBuffer.UV6Kind);
    ...
    loadAttribute('JOINTS_1', BABYLON.VertexBuffer.MatricesIndicesExtraKind);
    loadAttribute('WEIGHTS_1', BABYLON.VertexBuffer.MatricesWeightsExtraKind);
  ...
  private _createMorphTargets(context: string, node: INode, mesh: IMesh, primitive: IMeshPrimitive, babylonMesh: BABYLON.Mesh): void {
    ...
    this._babylonScene._blockEntityCollection = !!this._assetContainer;
    babylonMesh.morphTargetManager = new BABYLON.MorphTargetManager(this._babylonScene);
    ...
    for (let index = 0; index < primitive.targets.length; index++) {
      ...
      babylonMesh.morphTargetManager.addTarget(new BABYLON.MorphTarget(name, weight, babylonMesh.getScene()));
  ...
  private _loadMorphTargetsAsync(
    ...
    primitive: IMeshPrimitive,
    babylonMesh: BABYLON.Mesh,
    babylonGeometry: BABYLON.Geometry
  ): Promise<void> {
    if (!primitive.targets) {
      return Promise.resolve();
    }
  ...
  private _loadMorphTargetVertexDataAsync(context: string, babylonGeometry: BABYLON.Geometry, attributes: { [name: string]: number }, babylonMorphTarget: BABYLON.MorphTarget): Promise<void> {
    const promises = new Array<Promise<any>>();

    const loadAttribute = (attribute: string, kind: string, setData: (babylonVertexBuffer: BABYLON.VertexBuffer, data: Float32Array) => void) => {
      if (attributes[attribute] == undefined) {
        return;
      }
    ...
    loadAttribute('POSITION', BABYLON.VertexBuffer.PositionKind, (babylonVertexBuffer, data) => {
    ...
    loadAttribute('NORMAL', BABYLON.VertexBuffer.NormalKind, (babylonVertexBuffer, data) => {
    ...
    loadAttribute('TANGENT', BABYLON.VertexBuffer.TangentKind, (babylonVertexBuffer, data) => {
  ...
  private static _LoadTransform(node: INode, babylonNode: BABYLON.TransformNode): void {
    ...
    if (node.matrix) {
      const matrix = BABYLON.Matrix.FromArray(node.matrix);
  ...
  private _loadSkinAsync(context: string, node: INode, skin: ISkin, assign: (babylonSkeleton: BABYLON.Skeleton) => void): Promise<void> {
    ...
    this._babylonScene._blockEntityCollection = !!this._assetContainer;
    const babylonSkeleton = new BABYLON.Skeleton(skin.name || skeletonId, skeletonId, this._babylonScene);
  ...
  private _loadBones(context: string, skin: ISkin, babylonSkeleton: BABYLON.Skeleton): void {
      ...
      if (rootNode) {
        if (skin.skeleton === undefined) {
          ...
          if (skeletonNode !== rootNode && !isParent(skeletonNode, rootNode)) {
            BABYLON.Logger.Warn(`${context}/skeleton: Overriding with nearest common ancestor as skeleton node is not a common root`);
            skin.skeleton = rootNode.index;
          }
        }
      } else {
        BABYLON.Logger.Warn(`${context}: Failed to find common root`);
      }
    }

    const babylonBones: { [index: number]: BABYLON.Bone } = {};
  ...
  private _findSkeletonRootNode(context: string, joints: Array<number>): BABYLON.Nullable<INode> {
    if (joints.length === 0) {
      return null;
    }
    ...
    let rootNode: BABYLON.Nullable<INode> = null;
    for (let i = 0; ; ++i) {
  ...
  private _loadBone(
    ...
    babylonBones: { [index: number]: BABYLON.Bone }
  ): BABYLON.Bone {
    ...
    let parentBabylonBone: BABYLON.Nullable<BABYLON.Bone> = null;
    if (node.index !== skin.skeleton) {
      if (node.parent && node.parent.index !== -1) {
        parentBabylonBone = this._loadBone(node.parent, skin, babylonSkeleton, babylonBones);
      } else if (skin.skeleton !== undefined) {
        BABYLON.Logger.Warn(`/skins/${skin.index}/skeleton: Skeleton node is not a common root`);
      }
    }

    const boneIndex = skin.joints.indexOf(node.index);
    babylonBone = new BABYLON.Bone(node.name || `joint${node.index}`, babylonSkeleton, parentBabylonBone, this._getNodeMatrix(node), null, null, boneIndex);
  ...
  private _loadSkinInverseBindMatricesDataAsync(context: string, skin: ISkin): Promise<BABYLON.Nullable<Float32Array>> {
  ...
  private _updateBoneMatrices(babylonSkeleton: BABYLON.Skeleton, inverseBindMatricesData: BABYLON.Nullable<Float32Array>): void {
    for (const babylonBone of babylonSkeleton.bones) {
      const baseMatrix = BABYLON.Matrix.Identity();
  ...
  private _getNodeMatrix(node: INode): BABYLON.Matrix {
    return node.matrix
      ? BABYLON.Matrix.FromArray(node.matrix)
      : BABYLON.Matrix.Compose(
        node.scale ? BABYLON.Vector3.FromArray(node.scale) : BABYLON.Vector3.One(),
        node.rotation ? BABYLON.Quaternion.FromArray(node.rotation) : BABYLON.Quaternion.Identity(),
        node.translation ? BABYLON.Vector3.FromArray(node.translation) : BABYLON.Vector3.Zero()
      );
  }
  ...
  public loadCameraAsync(
    context: string,
    camera: ICamera,
    assign: (babylonCamera: BABYLON.Camera) => void = () => { }
  ): Promise<BABYLON.Camera> {
    ...
    this._babylonScene._blockEntityCollection = !!this._assetContainer;
    const babylonCamera = new BABYLON.FreeCamera(camera.name || `camera${camera.index}`, BABYLON.Vector3.Zero(), this._babylonScene, false);
   ...
   */
  public loadAnimationAsync(context: string, animation: IAnimation): Promise<BABYLON.AnimationGroup> {
    ...
    this._babylonScene._blockEntityCollection = !!this._assetContainer;
    const babylonAnimationGroup = new BABYLON.AnimationGroup(animation.name || `animation${animation.index}`, this._babylonScene);
  ...
  public _loadAnimationChannelFromTargetInfoAsync(
    ...
    return this._loadAnimationSamplerAsync(`${animationContext}/samplers/${channel.sampler}`, sampler).then((data) => {
      ...
      for (const property of targetInfo.properties) {
        ...
        const keys = new Array<BABYLON.IAnimationKey>(input.length);
        let outputOffset = 0;
  ...
  private _loadAccessorAsync(context: string, accessor: IAccessor, constructor: TypedArrayConstructor): Promise<ArrayBufferView> {
    ...
    const byteStride = numComponents * BABYLON.VertexBuffer.GetTypeByteLength(accessor.componentType);
      ...
      accessor._data = accessor._data.then((data) => {
        ...
        return Promise.all([
          ...
          if (accessor.componentType === AccessorComponentType.FLOAT && !accessor.normalized) {
            ...
            BABYLON.VertexBuffer.ForEach(sparseData, 0, byteStride, numComponents, accessor.componentType, values.length, accessor.normalized || false, (value, index) => {
              values[index] = value;
            });
          }
   ...
   * @internal
   */
  public _loadIndicesAccessorAsync(context: string, accessor: IAccessor): Promise<BABYLON.IndicesArray> {
    ...
    if (accessor._data) {
      return accessor._data as Promise<BABYLON.IndicesArray>;
    }
   ...
   * @internal
   */
  public _loadVertexBufferViewAsync(bufferView: IBufferView): Promise<BABYLON.Buffer> {
    ...
    bufferView._babylonBuffer = this.loadBufferViewAsync(`/bufferViews/${bufferView.index}`, bufferView).then((data) => {
      return new BABYLON.Buffer(engine, data, false);
    });
   ...
   * @internal
   */
  public _loadVertexAccessorAsync(context: string, accessor: IAccessor, kind: string): Promise<BABYLON.VertexBuffer> {
    ...
    if (accessor.sparse) {
      accessor._babylonVertexBuffer[kind] = this._loadFloatAccessorAsync(context, accessor).then((data) => {
        return new BABYLON.VertexBuffer(engine, data, kind, false);
      });
    }
    // Load joint indices as a float array since the shaders expect float data but glTF uses unsigned byte/short.
    // This prevents certain platforms (e.g. D3D) from having to convert the data to float on the fly.
    else if (kind === BABYLON.VertexBuffer.MatricesIndicesKind || kind === BABYLON.VertexBuffer.MatricesIndicesExtraKind) {
      accessor._babylonVertexBuffer[kind] = this._loadFloatAccessorAsync(context, accessor).then((data) => {
        return new BABYLON.VertexBuffer(engine, data, kind, false);
      });
    } else {
  ...
  private _loadMaterialMetallicRoughnessPropertiesAsync(context: string, properties: IMaterialPbrMetallicRoughness, babylonMaterial: BABYLON.Material): Promise<void> {
  ...
  private _createDefaultMaterial(name: string, babylonDrawMode: number): BABYLON.Material {
    this._babylonScene._blockEntityCollection = !!this._assetContainer;
    const babylonMaterial = new BABYLON.PBRMaterial(name, this._babylonScene);
   ...
   * @returns A promise that resolves when the load is complete
   */
  public loadMaterialPropertiesAsync(context: string, material: IMaterial, babylonMaterial: BABYLON.Material): Promise<void> {
   ...
   * @returns A promise that resolves when the load is complete
   */
  public loadMaterialBasePropertiesAsync(context: string, material: IMaterial, babylonMaterial: BABYLON.Material): Promise<void> {
    if (!(babylonMaterial instanceof BABYLON.PBRMaterial)) {
      throw new Error(`${context}: Material type not supported`);
    }
   ...
   * @param babylonMaterial The Babylon material
   */
  public loadMaterialAlphaProperties(context: string, material: IMaterial, babylonMaterial: BABYLON.Material): void {
    if (!(babylonMaterial instanceof BABYLON.PBRMaterial)) {
      throw new Error(`${context}: Material type not supported`);
    }
   ...
   */
  public loadTextureInfoAsync(
    context: string,
    textureInfo: ITextureInfo,
    assign: (babylonTexture: BABYLON.BaseTexture) => void = () => { }
  ...
  public _createTextureAsync(
    ...
    this._babylonScene._blockEntityCollection = !!this._assetContainer;
    const textureCreationOptions: BABYLON.ITextureCreationOptions = {
    ...
    const babylonTexture = new BABYLON.Texture(null, this._babylonScene, textureCreationOptions);
  ...
  public loadUriAsync(context: string, property: IProperty, uri: string): Promise<ArrayBufferView> {
    ...
    if (BABYLON.IsBase64DataUrl(uri)) {
      const data = new Uint8Array(BABYLON.DecodeBase64UrlToBinary(uri));
  ...
  private static _GetTextureWrapMode(context: string, mode: TextureWrapMode | undefined): number {
    ...
    switch (mode) {
      ...
      default:
        BABYLON.Logger.Warn(`${context}: Invalid value (${mode})`);
        return BABYLON.Texture.WRAP_ADDRESSMODE;
    }
  }
    ...
    if (magFilter === TextureMagFilter.LINEAR) {
      switch (minFilter) {
        case TextureMinFilter.NEAREST:
          return BABYLON.Texture.LINEAR_NEAREST;
        case TextureMinFilter.LINEAR:
          return BABYLON.Texture.LINEAR_LINEAR;
        case TextureMinFilter.NEAREST_MIPMAP_NEAREST:
          return BABYLON.Texture.LINEAR_NEAREST_MIPNEAREST;
        case TextureMinFilter.LINEAR_MIPMAP_NEAREST:
          return BABYLON.Texture.LINEAR_LINEAR_MIPNEAREST;
        case TextureMinFilter.NEAREST_MIPMAP_LINEAR:
          return BABYLON.Texture.LINEAR_NEAREST_MIPLINEAR;
        case TextureMinFilter.LINEAR_MIPMAP_LINEAR:
          return BABYLON.Texture.LINEAR_LINEAR_MIPLINEAR;
        default:
          BABYLON.Logger.Warn(`${context}/minFilter: Invalid value (${minFilter})`);
          return BABYLON.Texture.LINEAR_LINEAR_MIPLINEAR;
      }
    } else {
      if (magFilter !== TextureMagFilter.NEAREST) {
        BABYLON.Logger.Warn(`${context}/magFilter: Invalid value (${magFilter})`);
      }

      switch (minFilter) {
        case TextureMinFilter.NEAREST:
          return BABYLON.Texture.NEAREST_NEAREST;
        case TextureMinFilter.LINEAR:
          return BABYLON.Texture.NEAREST_LINEAR;
        case TextureMinFilter.NEAREST_MIPMAP_NEAREST:
          return BABYLON.Texture.NEAREST_NEAREST_MIPNEAREST;
        case TextureMinFilter.LINEAR_MIPMAP_NEAREST:
          return BABYLON.Texture.NEAREST_LINEAR_MIPNEAREST;
        case TextureMinFilter.NEAREST_MIPMAP_LINEAR:
          return BABYLON.Texture.NEAREST_NEAREST_MIPLINEAR;
        case TextureMinFilter.LINEAR_MIPMAP_LINEAR:
          return BABYLON.Texture.NEAREST_LINEAR_MIPLINEAR;
        default:
          BABYLON.Logger.Warn(`${context}/minFilter: Invalid value (${minFilter})`);
          return BABYLON.Texture.NEAREST_NEAREST_MIPNEAREST;
      }
  ...
  private static _GetTypedArray(
    ...
    const componentTypeLength = BABYLON.VertexBuffer.GetTypeByteLength(componentType);
    if (byteOffset % componentTypeLength !== 0) {
      // HACK: Copy the buffer if byte offset is not a multiple of component type byte length.
      BABYLON.Logger.Warn(`${context}: Copying buffer as byte offset (${byteOffset}) is not a multiple of component type byte length (${componentTypeLength})`);
  ...
  public static _GetDrawMode(context: string, mode: number | undefined): number {
    ...
    switch (mode) {
      case MeshPrimitiveMode.POINTS:
        return BABYLON.Material.PointListDrawMode;
      case MeshPrimitiveMode.LINES:
        return BABYLON.Material.LineListDrawMode;
      case MeshPrimitiveMode.LINE_LOOP:
        return BABYLON.Material.LineLoopDrawMode;
      case MeshPrimitiveMode.LINE_STRIP:
        return BABYLON.Material.LineStripDrawMode;
      case MeshPrimitiveMode.TRIANGLES:
        return BABYLON.Material.TriangleFillMode;
      case MeshPrimitiveMode.TRIANGLE_STRIP:
        return BABYLON.Material.TriangleStripDrawMode;
      case MeshPrimitiveMode.TRIANGLE_FAN:
        return BABYLON.Material.TriangleFanDrawMode;
    }
  ...
  private _applyExtensions<T>(property: IProperty, functionName: string, actionAsync: (extension: IGLTFLoaderExtension) => BABYLON.Nullable<T> | undefined): BABYLON.Nullable<T> {
    for (const extension of this._extensions) {
      if (extension.enabled) {
  ...
  private _extensionsLoadSceneAsync(context: string, scene: IScene): BABYLON.Nullable<Promise<void>> {
    return this._applyExtensions(
      scene, 'loadScene', (extension) => extension.loadSceneAsync && extension.loadSceneAsync(context, scene));
  }
  ...
  private _extensionsLoadNodeAsync(
    ...
    node: INode,
    assign: (babylonTransformNode: BABYLON.TransformNode) => void
  ): BABYLON.Nullable<Promise<BABYLON.TransformNode>> {
    return this._applyExtensions(
      node, 'loadNode', (extension) => extension.loadNodeAsync && extension.loadNodeAsync(context, node, assign));
  }
  ...
  private _extensionsLoadCameraAsync(
    ...
    camera: ICamera,
    assign: (babylonCamera: BABYLON.Camera) => void
  ): BABYLON.Nullable<Promise<BABYLON.Camera>> {
    return this._applyExtensions(
      camera, 'loadCamera', (extension) => extension.loadCameraAsync && extension.loadCameraAsync(context, camera, assign));
  }
  ...
  private _extensionsLoadVertexDataAsync(
    ...
    primitive: IMeshPrimitive,
    babylonMesh: BABYLON.Mesh
  ): BABYLON.Nullable<Promise<BABYLON.Geometry>> {
    return this._applyExtensions(
  ...
  private _extensionsCreateMaterial(context: string, material: IMaterial, babylonDrawMode: number): BABYLON.Nullable<BABYLON.Material> {
    return this._applyExtensions(material, 'createMaterial', (extension) => extension.createMaterial && extension.createMaterial(context, material, babylonDrawMode));
  }

  private _extensionsLoadMaterialPropertiesAsync(context: string, material: IMaterial, babylonMaterial: BABYLON.Material): BABYLON.Nullable<Promise<void>> {
    return this._applyExtensions(
      material,
  ...
  private _extensionsLoadTextureInfoAsync(context: string, textureInfo: ITextureInfo, assign: (babylonTexture: BABYLON.BaseTexture) => void): BABYLON.Nullable<Promise<BABYLON.BaseTexture>> {
    return this._applyExtensions(textureInfo, 'loadTextureInfo', (extension) => extension.loadTextureInfoAsync && extension.loadTextureInfoAsync(context, textureInfo, assign));
  }

  private _extensionsLoadTextureAsync(context: string, texture: ITexture, assign: (babylonTexture: BABYLON.BaseTexture) => void): BABYLON.Nullable<Promise<BABYLON.BaseTexture>> {
    return this._applyExtensions(texture, 'loadTexture', (extension) => extension._loadTextureAsync && extension._loadTextureAsync(context, texture, assign));
  }

  private _extensionsLoadAnimationAsync(context: string, animation: IAnimation): BABYLON.Nullable<Promise<BABYLON.AnimationGroup>> {
  ...
  private _extensionsLoadSkinAsync(context: string, node: INode, skin: ISkin): BABYLON.Nullable<Promise<void>> {
    return this._applyExtensions(skin, 'loadSkin', (extension) => extension._loadSkinAsync && extension._loadSkinAsync(context, node, skin));
  }

  private _extensionsLoadUriAsync(context: string, property: IProperty, uri: string): BABYLON.Nullable<Promise<ArrayBufferView>> {
    return this._applyExtensions(property, 'loadUri', (extension) => extension._loadUriAsync && extension._loadUriAsync(context, property, uri));
  }

  private _extensionsLoadBufferViewAsync(context: string, bufferView: IBufferView): BABYLON.Nullable<Promise<ArrayBufferView>> {
    return this._applyExtensions(bufferView, 'loadBufferView', (extension) => extension.loadBufferViewAsync && extension.loadBufferViewAsync(context, bufferView));
  }

  private _extensionsLoadBufferAsync(context: string, buffer: IBuffer, byteOffset: number, byteLength: number): BABYLON.Nullable<Promise<ArrayBufferView>> {
  ...
  public static LoadExtensionAsync<TExtension = any, TResult = void>(
    ...
    actionAsync: (extensionContext: string, extension: TExtension) => BABYLON.Nullable<Promise<TResult>>
  ): BABYLON.Nullable<Promise<TResult>> {
  ...
  public static LoadExtraAsync<TExtra = any, TResult = void>(
    ...
    property: IProperty,
    extensionName: string,
    actionAsync: (extraContext: string, extra: TExtra) => BABYLON.Nullable<Promise<TResult>>
  ): BABYLON.Nullable<Promise<TResult>> {
    if (!property.extras) {
      return null;
    }
