...
export class GLTFFileLoader implements BABYLON.IDisposable, BABYLON.ISceneLoaderPluginAsync, BABYLON.ISceneLoaderPluginFactory {
   ...
   */
  public onParsedObservable = new BABYLON.Observable<IGLTFLoaderData>();

  private _onParsedObserver: BABYLON.Nullable<BABYLON.Observer<IGLTFLoaderData>>;
   ...
   */
  public readonly onMeshLoadedObservable = new BABYLON.Observable<BABYLON.AbstractMesh>();

  private _onMeshLoadedObserver: BABYLON.Nullable<BABYLON.Observer<BABYLON.AbstractMesh>>;

  /**
   * Callback raised when the loader creates a mesh after parsing the glTF properties of the mesh.
   * Note that the callback is called as soon as the mesh object is created, meaning some data may not have been setup yet for this mesh (vertex data, morph targets, material, ...)
   */
  public set onMeshLoaded(callback: (mesh: BABYLON.AbstractMesh) => void) {
   ...
   */
  public readonly onSkinLoadedObservable = new BABYLON.Observable<{ node: BABYLON.TransformNode; skinnedNode: BABYLON.TransformNode }>();

  /**
   * Observable raised when the loader creates a texture after parsing the glTF properties of the texture.
   */
  public readonly onTextureLoadedObservable = new BABYLON.Observable<BABYLON.BaseTexture>();

  private _onTextureLoadedObserver: BABYLON.Nullable<BABYLON.Observer<BABYLON.BaseTexture>>;

  /**
   * Callback raised when the loader creates a texture after parsing the glTF properties of the texture.
   */
  public set onTextureLoaded(callback: (texture: BABYLON.BaseTexture) => void) {
   ...
   */
  public readonly onMaterialLoadedObservable = new BABYLON.Observable<BABYLON.Material>();

  private _onMaterialLoadedObserver: BABYLON.Nullable<BABYLON.Observer<BABYLON.Material>>;

  /**
   * Callback raised when the loader creates a material after parsing the glTF properties of the material.
   */
  public set onMaterialLoaded(callback: (material: BABYLON.Material) => void) {
   ...
   */
  public readonly onCameraLoadedObservable = new BABYLON.Observable<BABYLON.Camera>();

  private _onCameraLoadedObserver: BABYLON.Nullable<BABYLON.Observer<BABYLON.Camera>>;

  /**
   * Callback raised when the loader creates a camera after parsing the glTF properties of the camera.
   */
  public set onCameraLoaded(callback: (camera: BABYLON.Camera) => void) {
   ...
   */
  public readonly onCompleteObservable = new BABYLON.Observable<void>();

  private _onCompleteObserver: BABYLON.Nullable<BABYLON.Observer<void>>;
   ...
   */
  public readonly onErrorObservable = new BABYLON.Observable<any>();

  private _onErrorObserver: BABYLON.Nullable<BABYLON.Observer<any>>;
   ...
   */
  public readonly onDisposeObservable = new BABYLON.Observable<void>();

  private _onDisposeObserver: BABYLON.Nullable<BABYLON.Observer<void>>;
   ...
   */
  public readonly onExtensionLoadedObservable = new BABYLON.Observable<IGLTFLoaderExtension>();

  private _onExtensionLoadedObserver: BABYLON.Nullable<BABYLON.Observer<IGLTFLoaderExtension>>;
   ...
   */
  public readonly onValidatedObservable = new BABYLON.Observable<GLTF2.IGLTFValidationResults>();

  private _onValidatedObserver: BABYLON.Nullable<BABYLON.Observer<GLTF2.IGLTFValidationResults>>;
  ...
  private _loader: BABYLON.Nullable<IGLTFLoader> = null;
  private _state: BABYLON.Nullable<GLTFLoaderState> = null;
  private _progressCallback?: (event: BABYLON.ISceneLoaderProgressEvent) => void;
  ...
  public loadFile(
    ...
    onProgress?: (ev: BABYLON.ISceneLoaderProgressEvent) => void,
    useArrayBuffer?: boolean,
    onError?: (request?: BABYLON.WebRequest, exception?: BABYLON.LoadFileError) => void,
    name?: string
  ): BABYLON.Nullable<IFileRequest> {
    ...
    const fileName = (fileOrUrl as File).name || BABYLON.Tools.GetFilename(fileOrUrl as string);

    if (useArrayBuffer) {
      if (this.useRangeRequests) {
        if (this.validate) {
          BABYLON.Logger.Warn('glTF validation is not supported when range requests are enabled');
        }

        const fileRequest: IFileRequest = {
          abort: () => { },
          onCompleteObservable: new BABYLON.Observable<IFileRequest>(),
        };
        ...
        this._unpackBinaryAsync(new BABYLON.DataReader(dataBuffer)).then(
          (loaderData) => {
   ...
   * @internal
   */
  public loadAsync(scene: BABYLON.Scene, data: any, rootUrl: string, onProgress?: (event: BABYLON.ISceneLoaderProgressEvent) => void, fileName?: string): Promise<void> {
    return Promise.resolve().then(() => {
      this.onParsedObservable.notifyObservers(data);
      this.onParsedObservable.clear();
  ...
  public loadAssetContainerAsync(
    ...
    return Promise.resolve().then(() => {
      ...
      const materials: Array<BABYLON.Material> = [];
      this.onMaterialLoadedObservable.add((material) => {
        materials.push(material);
      });
      const textures: Array<BABYLON.BaseTexture> = [];
      this.onTextureLoadedObservable.add((texture) => {
        textures.push(texture);
      });
      const cameras: Array<BABYLON.Camera> = [];
      this.onCameraLoadedObservable.add((camera) => {
        cameras.push(camera);
      });

      const morphTargetManagers: Array<BABYLON.MorphTargetManager> = [];
   ...
   * @internal
   */
  public directLoad(scene: BABYLON.Scene, data: string): Promise<any> {
    if (
      ...
      const arrayBuffer = BABYLON.DecodeBase64UrlToBinary(data);
  ...
  /** @internal */
  public createPlugin(): BABYLON.ISceneLoaderPlugin | BABYLON.ISceneLoaderPluginAsync {
   ...
   */
  public get loaderState(): BABYLON.Nullable<GLTFLoaderState> {
    return this._state;
  }
   ...
   */
  public onLoaderStateChangedObservable = new BABYLON.Observable<BABYLON.Nullable<GLTFLoaderState>>();
  ...
  public _loadFile(
    ...
    useArrayBuffer?: boolean,
    onError?: (request?: BABYLON.WebRequest) => void,
    onOpened?: (request: BABYLON.WebRequest) => void
  ): IFileRequest {
  ...
  private _validate(scene: BABYLON.Scene, data: string | ArrayBufferView, rootUrl = '', fileName = ''): void {
    if (!this.validate) {
      return;
    }
    ...
    GLTFValidation.ValidateAsync(data, rootUrl, fileName, (uri) => {
      ...
      (reason) => {
        this._endPerformanceCounter('Validate JSON');
        BABYLON.Tools.Warn(`Failed to validate: ${reason.message}`);
        this.onValidatedObservable.clear();
      }
    );
  }
  ...
  private _unpackBinaryAsync(dataReader: BABYLON.DataReader): Promise<IGLTFLoaderData> {
    ...
    return dataReader.loadAsync(20).then(() => {
      ...
      if (magic !== Binary.Magic) {
        throw new BABYLON.RuntimeError("Unexpected magic: " + magic, BABYLON.ErrorCodes.GLTFLoaderUnexpectedMagicError);
      }
      ...
      if (!this.useRangeRequests && length !== dataReader.buffer.byteLength) {
        BABYLON.Logger.Warn(`Length in header does not match actual data length: ${length} != ${dataReader.buffer.byteLength}`);
      }
  ...
  private _unpackBinaryV1Async(dataReader: BABYLON.DataReader, length: number): Promise<IGLTFLoaderData> {
    const ContentFormat = {
      JSON: 0,
    };
  ...
  private _unpackBinaryV2Async(dataReader: BABYLON.DataReader, length: number): Promise<IGLTFLoaderData> {
    const ChunkFormat = {
      JSON: 0x4e4f534a,
      BIN: 0x004e4942,
    };
  ...
  private static _parseVersion(version: string): BABYLON.Nullable<{ major: number; minor: number }> {
  ...
  private _logEnabled(message: string): void {
    ...
    BABYLON.Logger.Log(`${spaces}${message}`);
  }
