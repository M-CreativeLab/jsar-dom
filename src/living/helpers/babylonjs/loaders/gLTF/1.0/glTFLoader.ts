...
type FloatArray = BABYLON.FloatArray;
type Quaternion = BABYLON.Quaternion;
type Vector3 = BABYLON.Vector3;
type Matrix = BABYLON.Matrix;
type VertexData = BABYLON.VertexData;
type Node = BABYLON.Node;
type Scene = BABYLON.Scene;
type Animation = BABYLON.Animation;
type VertexBuffer = BABYLON.VertexBuffer;
...
const loadAnimations = (gltfRuntime: IGLTFRuntime) => {
  for (const anim in gltfRuntime.animations) {
    ...
    for (let i = 0; i < animation.channels.length; i++) {
      ...
      const isBone = targetNode instanceof BABYLON.Bone;
      ...
      for (let j = 0; j < bufferInput.length; j++) {
        ...
        if (isBone) {
          const bone = <BABYLON.Bone>targetNode;
 ...
 * @param newSkeleton
 */
const getParentBone = (gltfRuntime: IGLTFRuntime, skins: IGLTFSkins, jointName: string, newSkeleton: BABYLON.Skeleton): Nullable<BABYLON.Bone> => {
  // Try to find
  ...
  for (const nde in nodes) {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (child.jointName === jointName) {
        ...
        const bone = new BABYLON.Bone(node.name || '', newSkeleton, getParentBone(gltfRuntime, skins, node.jointName, newSkeleton), mat);
        bone.id = nde;
        return bone;
      }
    }
 ...
 * @param id
 */
const getNodeToRoot = (nodesToRoot: INodeToRoot[], id: string): Nullable<BABYLON.Bone> => {
 ...
 * @param nodesToRoot
 */
const getNodesToRoot = (gltfRuntime: IGLTFRuntime, newSkeleton: BABYLON.Skeleton, skins: IGLTFSkins, nodesToRoot: INodeToRoot[]) => {
  ...
  for (const nde in gltfRuntime.nodes) {
    ...
    const mat = configureBoneTransformation(node);
    const bone = new BABYLON.Bone(node.name || '', newSkeleton, null, mat);
    bone.id = id;
 ...
 * @param newSkeleton
 */
const importSkeleton = (gltfRuntime: IGLTFRuntime, skins: IGLTFSkins, mesh: BABYLON.Mesh, newSkeleton: BABYLON.Skeleton | undefined): BABYLON.Skeleton => {
  if (!newSkeleton) {
  ...
  const nodesToRootToAdd: BABYLON.Bone[] = [];
  ...
  for (let i = 0; i < skins.jointNames.length; i++) {
    ...
    let foundBone = false;
    let parentBone: Nullable<BABYLON.Bone> = null;
    ...
    const bone = new BABYLON.Bone(node.jointName || '', newSkeleton, parentBone, mat);
    bone.id = id;
  }
 ...
 * @param newMesh
 */
const importMesh = (gltfRuntime: IGLTFRuntime, node: IGLTFNode, meshes: string[], id: string, newMesh: BABYLON.Mesh): BABYLON.Mesh => {
  if (!newMesh) {
    gltfRuntime.scene._blockEntityCollection = !!gltfRuntime.assetContainer;
  ...
  const subMaterials: BABYLON.Material[] = [];
  ...
  let material: BABYLON.StandardMaterial | BABYLON.MultiMaterial;
  gltfRuntime.scene._blockEntityCollection = !!gltfRuntime.assetContainer;
  if (subMaterials.length > 1) {
    material = new BABYLON.MultiMaterial('multimat' + id, gltfRuntime.scene);
    (material as BABYLON.MultiMaterial).subMaterials = subMaterials;
  } else {
    material = new BABYLON.StandardMaterial('multimat' + id, gltfRuntime.scene);
  }

  if (subMaterials.length === 1) {
    material = subMaterials[0] as BABYLON.StandardMaterial;
  }
 ...
 * @param node
 */
const configureNodeFromMatrix = (newNode: BABYLON.Mesh, node: IGLTFNode) => {
  if (node.matrix) {
...
const importNode = (gltfRuntime: IGLTFRuntime, node: IGLTFNode, id: string): Nullable<Node> => {
  ...
  if (node.skin) {
    if (node.meshes) {
      ...
      const newMesh = importMesh(gltfRuntime, node, node.meshes, id, <BABYLON.Mesh>node.babylonNode);
     ...
     */
    const newMesh = importMesh(gltfRuntime, node, node.mesh ? [node.mesh] : node.meshes, id, <BABYLON.Mesh>node.babylonNode);
    lastNode = newMesh;
  }
  // Lights
    ...
    if (light) {
      if (light.type === 'ambient') {
        ...
        const hemiLight = new BABYLON.HemisphericLight(node.light, Vector3.Zero(), gltfRuntime.scene);
        ...
        const dirLight = new BABYLON.DirectionalLight(node.light, Vector3.Zero(), gltfRuntime.scene);
        ...
        const ptLight = new BABYLON.PointLight(node.light, Vector3.Zero(), gltfRuntime.scene);
        ...
        const spLight = new BABYLON.SpotLight(node.light, Vector3.Zero(), Vector3.Zero(), 0, 0, gltfRuntime.scene);
  ...
  // Cameras
    ...
    if (camera) {
      ...
      if (camera.type === 'orthographic') {
        const orthoCamera = new BABYLON.FreeCamera(node.camera, Vector3.Zero(), gltfRuntime.scene, false);
        ...
        const persCamera = new BABYLON.FreeCamera(node.camera, Vector3.Zero(), gltfRuntime.scene, false);
  ...
  if (!node.jointName) {
    if (node.babylonNode) {
      ...
      gltfRuntime.scene._blockEntityCollection = !!gltfRuntime.assetContainer;
      const dummy = new BABYLON.Mesh(node.name || '', gltfRuntime.scene);
...
const onBindShaderMaterial = (
  ...
  onSuccess: (shaderMaterial: BABYLON.ShaderMaterial) => void
) => {
  ...
  for (const unif in unTreatedUniforms) {
    ...
    if (type === EParameterType.FLOAT_MAT2 || type === EParameterType.FLOAT_MAT3 || type === EParameterType.FLOAT_MAT4) {
      if (uniform.semantic && !uniform.source && !uniform.node) {
        GLTFUtils.SetMatrix(gltfRuntime.scene, mesh, uniform, unif, <BABYLON.Effect>shaderMaterial.getEffect());
        ...
        GLTFUtils.SetMatrix(gltfRuntime.scene, source, uniform, unif, <BABYLON.Effect>shaderMaterial.getEffect());
      }
    } else {
      ...
      if (type === EParameterType.SAMPLER_2D) {
        ...
        (<BABYLON.Effect>shaderMaterial.getEffect()).setTexture(unif, texture);
      } else {
        GLTFUtils.SetUniform(<BABYLON.Effect>shaderMaterial.getEffect(), unif, value, type);
      }
    }
  }
...
const prepareShaderMaterialUniforms = (
  ...
  for (const unif in unTreatedUniforms) {
    ...
    const onLoadTexture = (uniformName: Nullable<string>) => {
      return (texture: BABYLON.Texture) => {
 ...
 * @param onError
 */
const onShaderCompileError = (program: IGLTFProgram, shaderMaterial: BABYLON.ShaderMaterial, onError: (message: string) => void) => {
  return (effect: BABYLON.Effect, error: string) => {
...
const onShaderCompileSuccess = (
  ...
  return (_: BABYLON.Effect) => {
    prepareShaderMaterialUniforms(gltfRuntime, shaderMaterial, technique, material, unTreatedUniforms);

    shaderMaterial.onBind = (mesh: BABYLON.AbstractMesh) => {
...
export class GLTFLoaderBase {
  ...
  public static LoadMaterialAsync(gltfRuntime: IGLTFRuntime, id: string, onSuccess: (material: BABYLON.Material) => void, onError: (message: string) => void): void {
    ...
    if (!technique) {
      gltfRuntime.scene._blockEntityCollection = !!gltfRuntime.assetContainer;
      const defaultMaterial = new BABYLON.StandardMaterial(id, gltfRuntime.scene);
    ...
    const vertexShader: string = BABYLON.Effect.ShadersStore[program.vertexShader + 'VertexShader'];
    const pixelShader: string = BABYLON.Effect.ShadersStore[program.fragmentShader + 'PixelShader'];
    let newVertexShader = '';
    let newPixelShader = '';
    ...
    const shaderMaterial = new BABYLON.ShaderMaterial(id, gltfRuntime.scene, shaderPath, options);
...
export class GLTFLoader implements IGLTFLoader {
  ...
  private _importMeshAsync(
    ...
    rootUrl: string,
    assetContainer: Nullable<BABYLON.AssetContainer>,
    onSuccess: (meshes: BABYLON.AbstractMesh[], skeletons: BABYLON.Skeleton[]) => void,
      ...
      (gltfRuntime) => {
        ...
        for (const nde in gltfRuntime.nodes) {
          ...
          if (node.babylonNode instanceof BABYLON.AbstractMesh) {
            meshes.push(<BABYLON.AbstractMesh>node.babylonNode);
          }
        }
    ...
    const processShader = (sha: string, shader: IGLTFShader) => {
      GLTFLoaderExtension.LoadShaderStringAsync(
        ...
        (shaderString) => {
          ...
          if (shaderString) {
            BABYLON.Effect.ShadersStore[sha + (shader.type === EShaderType.VERTEX ? 'VertexShader' : 'PixelShader')] = shaderString;
          }
