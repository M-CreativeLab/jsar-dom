...
export class MTLFileLoader {
  ...
  public parseMTL(scene: Scene, data: string | ArrayBuffer, rootUrl: string, assetContainer: Nullable<AssetContainer>): void {
    ...
    let color: number[];
    //New material
    let material: Nullable<BABYLON.StandardMaterial> = null;
   ...
   * @returns The Texture
   */
  private static _GetTexture(rootUrl: string, value: string, scene: Scene): Nullable<BABYLON.Texture> {
    if (!value) {
      return null;
    }
    ...
    return new BABYLON.Texture(url, scene, false, MTLFileLoader.INVERT_TEXTURE_Y);
  }
}
