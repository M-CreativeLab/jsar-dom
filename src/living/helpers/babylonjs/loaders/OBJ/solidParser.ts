...
type IndicesArray = BABYLON.IndicesArray;
...
export class SolidParser {
  ...
  private _positions: Array<BABYLON.Vector3> = []; //values for the positions of vertices
  private _normals: Array<BABYLON.Vector3> = []; //Values for the normals
  private _uvs: Array<BABYLON.Vector2> = []; //Values for the textures
  private _colors: Array<BABYLON.Color4> = [];
  ...
  private _wrappedPositionForBabylon: Array<BABYLON.Vector3> = []; //The list of position in vectors
  private _wrappedUvsForBabylon: Array<BABYLON.Vector2> = []; //Array with all value of uvs to match with the indices
  private _wrappedColorsForBabylon: Array<BABYLON.Color4> = []; // Array with all color values to match with the indices
  private _wrappedNormalsForBabylon: Array<BABYLON.Vector3> = []; //Array with all value of normals to match with the indices
  ...
  private _grayColor = new BABYLON.Color4(0.5, 0.5, 0.5, 1);
  private _materialToUse: string[];
  private _babylonMeshesArray: Array<BABYLON.Mesh>;
   ...
   */
  public constructor(materialToUse: string[], babylonMeshesArray: Array<BABYLON.Mesh>, loadingOptions: OBJLoadingOptions) {
  ...
  private _setData(
    ...
    positionVectorFromOBJ: BABYLON.Vector3,
    textureVectorFromOBJ: BABYLON.Vector2,
    normalsVectorFromOBJ: BABYLON.Vector3,
    positionColorsFromOBJ?: BABYLON.Color4
  ) {
  ...
  public parse(meshesNames: any, data: string, scene: Scene, assetContainer: Nullable<AssetContainer>, onFileToLoadFound: (fileToLoad: string) => void): void {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.length === 0 || line.charAt(0) === "#") {
        ...
        this._positions.push(new BABYLON.Vector3(parseFloat(result[1]), parseFloat(result[2]), parseFloat(result[3])));
        ...
        if (this._loadingOptions.importVertexColors) {
          if (result.length >= 7) {
            ...
            this._colors.push(
              new BABYLON.Color4(r > 1 ? r / 255 : r, g > 1 ? g / 255 : g, b > 1 ? b / 255 : b, result.length === 7 || result[7] === undefined ? 1 : parseFloat(result[7]))
            );
          } else {
        ...
        this._normals.push(new BABYLON.Vector3(parseFloat(result[1]), parseFloat(result[2]), parseFloat(result[3])));
      } else if ((result = SolidParser.UVPattern.exec(line)) !== null) {
        //Create a Vector2 with the normals u, v
        //Value of result
        // ["vt 0.1 0.2 0.3", "0.1", "0.2"]
        //Add the Vector in the list of uvs
        this._uvs.push(new BABYLON.Vector2(parseFloat(result[1]) * this._loadingOptions.UVScaling.x, parseFloat(result[2]) * this._loadingOptions.UVScaling.y));
    ...
    if (!this._hasMeshes) {
      let newMaterial: Nullable<BABYLON.StandardMaterial> = null;
      if (this._indicesForBabylon.length) {
        ...
        if (!this._materialNameFromObj) {
          ...
          newMaterial = new BABYLON.StandardMaterial(BABYLON.Geometry.RandomId(), scene);
          ...
          if (!this._normals.length) {
            ...
            newMaterial.emissiveColor = BABYLON.Color3.White();
          }
        }
      }
    ...
    for (let j = 0; j < this._meshesFromObj.length; j++) {
      ...
      const babylonMesh = new BABYLON.Mesh(this._meshesFromObj[j].name, scene);
      ...
      const vertexData: BABYLON.VertexData = new BABYLON.VertexData(); //The container for the values
      ...
      if (this._loadingOptions.computeNormals) {
        ...
        BABYLON.VertexData.ComputeNormals(this._handledMesh.positions, this._handledMesh.indices, normals);
