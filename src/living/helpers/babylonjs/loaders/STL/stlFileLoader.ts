...
export class STLFileLoader implements BABYLON.ISceneLoaderPlugin {
  ...
  public importMesh(meshesNames: any, scene: Scene, data: any, rootUrl: string, meshes: Nullable<AbstractMesh[]>): boolean {
    ...
    if (typeof data !== 'string') {
      if (this._isBinary(data)) {
        // binary .stl
        const babylonMesh = new BABYLON.Mesh('stlmesh', scene);
    ...
    while ((matches = this.solidPattern.exec(data))) {
      ...
      const babylonMesh = new BABYLON.Mesh(meshName, scene);
   ...
   */
  public loadAssetContainer(scene: Scene, data: string, rootUrl: string): BABYLON.AssetContainer {
  ...
  private _parseBinary(mesh: BABYLON.Mesh, data: ArrayBuffer) {
    const reader = new DataView(data);
  ...
  private _parseASCII(mesh: BABYLON.Mesh, solidData: string) {
    const positions = [];
    const normals = [];
    const indices = [];
    let indicesCount = 0;
