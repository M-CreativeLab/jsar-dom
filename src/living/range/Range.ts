...
export function compareBoundaryPointsPosition(bpA: BoundaryPoint, bpB: BoundaryPoint) {
...
export function validateSetBoundaryPoint(node, offset) {
  if (node.nodeType === NodeImpl.prototype.DOCUMENT_TYPE_NODE) {
...
export function setBoundaryPointStart(range, node, offset) {
...
export function setBoundaryPointEnd(range, node, offset) {
  validateSetBoundaryPoint(node, offset);
...
function insertNodeInRange(node, range) {
  ...
  if (
    startNode.nodeType === NodeImpl.prototype.PROCESSING_INSTRUCTION_NODE ||
    startNode.nodeType === NodeImpl.prototype.COMMENT_NODE ||
    (startNode.nodeType === NodeImpl.prototype.TEXT_NODE && !domSymbolTree.parent(startNode)) ||
    node === startNode
  ) {
    throw new DOMException('Invalid start node.', 'HierarchyRequestError');
  }

  let referenceNode = startNode.nodeType === NodeImpl.prototype.TEXT_NODE ?
    startNode :
  ...
  if (startNode.nodeType === NodeImpl.prototype.TEXT_NODE) {
  ...
  newOffset += node.nodeType === NodeImpl.prototype.DOCUMENT_FRAGMENT_NODE ? nodeLength(node) : 1;
...
function cloneRange(range) {
  ...
  if (
    ...
    (
      originalStart.node.nodeType === NodeImpl.prototype.TEXT_NODE ||
      originalStart.node.nodeType === NodeImpl.prototype.PROCESSING_INSTRUCTION_NODE ||
      originalStart.node.nodeType === NodeImpl.prototype.COMMENT_NODE
    )
  ) {
  ...
  const hasDoctypeChildren = containedChildren.some(node => node.nodeType === NodeImpl.prototype.DOCUMENT_TYPE_NODE);
  ...
  if (
    ...
    (
      firstPartialContainedChild.nodeType === NodeImpl.prototype.TEXT_NODE ||
      firstPartialContainedChild.nodeType === NodeImpl.prototype.PROCESSING_INSTRUCTION_NODE ||
      firstPartialContainedChild.nodeType === NodeImpl.prototype.COMMENT_NODE
    )
  ) {
    const cloned = clone(originalStart.node);
    ...
    const subrange = new RangeImpl(_globalObject, [], {
  ...
  if (
    ...
    (
      lastPartiallyContainedChild.nodeType === NodeImpl.prototype.TEXT_NODE ||
      lastPartiallyContainedChild.nodeType === NodeImpl.prototype.PROCESSING_INSTRUCTION_NODE ||
      lastPartiallyContainedChild.nodeType === NodeImpl.prototype.COMMENT_NODE
    )
  ) {
...
function nextNodeDescendant(node: Node) {
...
export class RangeImpl extends AbstractRangeImpl implements Range {
  ...
  deleteContents() {
    ...
    if (
      ...
      const nodeAsCharacterData = originalStart.node as CharacterData;
      nodeAsCharacterData.replaceData(originalStart.offset, originalEnd.offset - originalStart.offset, '');
      return;
    }
    ...
    if (
      ...
      const nodeAsCharacterData = originalStart.node as CharacterData;
      nodeAsCharacterData.replaceData(originalStart.offset, nodeLength(originalStart.node) - originalStart.offset, '');
    }
    ...
    if (
      originalEnd.node.nodeType === NodeImpl.prototype.TEXT_NODE ||
      originalEnd.node.nodeType === NodeImpl.prototype.PROCESSING_INSTRUCTION_NODE ||
      originalEnd.node.nodeType === NodeImpl.prototype.COMMENT_NODE
    ) {
      const nodeAsCharacterData = originalStart.node as CharacterData;
      nodeAsCharacterData.replaceData(0, originalEnd.offset, '');
    }
  ...
  surroundContents(newParent) {
    ...
    while (node !== endNode) {
      if (node.nodeType !== NodeImpl.prototype.TEXT_NODE && isPartiallyContained(node, this)) {
        throw new DOMException(
    ...
    if (
      newParent.nodeType === NodeImpl.prototype.DOCUMENT_NODE ||
      newParent.nodeType === NodeImpl.prototype.DOCUMENT_TYPE_NODE ||
      newParent.nodeType === NodeImpl.prototype.DOCUMENT_FRAGMENT_NODE
    ) {
  ...
  cloneRange() {
    const { _start, _end, _hostObject } = this;
    return new RangeImpl(_hostObject, [], {
  ...
  toString() {
    ...
    if (_start.node === _end.node && _start.node.nodeType === NodeImpl.prototype.TEXT_NODE) {
      const nodeAsText = _start.node as Text;
      return nodeAsText.data.slice(_start.offset, _end.offset);
    }

    if (_start.node.nodeType === NodeImpl.prototype.TEXT_NODE) {
      const nodeAsText = _start.node as Text;
      s += nodeAsText.data.slice(_start.offset);
    }

    let currentNode = _start.node;
    const endNode = nextNodeDescendant(_end.node);
    while (currentNode && currentNode !== endNode) {
      if (currentNode.nodeType === NodeImpl.prototype.TEXT_NODE && isContained(currentNode, this)) {
        s += (currentNode as Text).data;
      }

      currentNode = domSymbolTree.following(currentNode);
    }

    if (_end.node.nodeType === NodeImpl.prototype.TEXT_NODE) {
      s += (_end.node as Text).data.slice(0, _end.offset);
    }
    return s;
  }
  ...
  createContextualFragment(fragment) {
    ...
    switch (node.nodeType) {
      case NodeImpl.prototype.DOCUMENT_NODE:
      case NodeImpl.prototype.DOCUMENT_FRAGMENT_NODE:
        element = null;
        break;

      case NodeImpl.prototype.ELEMENT_NODE:
        element = node;
        break;

      case NodeImpl.prototype.TEXT_NODE:
      case NodeImpl.prototype.COMMENT_NODE:
    ...
    if (
      ...
      // TODO
      // element = createElement(node.ownerDocument, 'body', HTML_NS);
    }
    // TODO
  ...
  _setLiveRangeStart(node, offset) {
    if (
      ...
      (this._start.node as unknown as NodeImpl)._referencedRanges.delete(this as unknown as Range);
    }
  ...
  _setLiveRangeEnd(node, offset) {
    if (
      ...
      (this._end.node as unknown as NodeImpl)._referencedRanges.delete(this as unknown as Range);
    }
