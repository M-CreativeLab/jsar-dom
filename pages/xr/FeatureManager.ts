 ...
 */
export interface IWebXRFeature extends BABYLON.IDisposable {
  /**
   ...
   */
  onFeatureAttachObservable: BABYLON.Observable<IWebXRFeature>;
  /**
   * Triggered when the feature is detached
   */
  onFeatureDetachObservable: BABYLON.Observable<IWebXRFeature>;
}
 ...
 */
export class WebXRFeaturesManager implements BABYLON.IDisposable {
  ...
  public static AddWebXRFeature(featureName: string, constructorFunction: WebXRFeatureConstructor, version: number = 1, stable: boolean = false) {
    this._AvailableFeatures[featureName] = this._AvailableFeatures[featureName] || { latest: version, stable: undefined };
  ...
  public enableFeature(
    ...
    if (constructed.isCompatible()) {
      ...
      if (required) {
        ...
        BABYLON.Tools.Warn(`Feature ${name} not compatible with the current environment/browser and was not enabled.`);
        return constructed;
      }
    }
  }
