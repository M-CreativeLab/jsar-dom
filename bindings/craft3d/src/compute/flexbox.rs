...
use crate::style::{
  AlignContent, AlignItems, AlignSelf, AvailableSpace, Dimension, Display, FlexWrap,
...
fn compute_preliminary(
  ...
  if let Some(inner_main_size) = constants.node_inner_size.main(constants.dir) {
    ...
    let new_gap = style
      .gap
      .main(constants.dir)
      .maybe_resolve(inner_container_size)
      .unwrap_or(0.0);
  ...
  tree
    ...
    .map(|(index, child, child_style)| {
      ...
      FlexItem {
        ...
        inset: child_style
          .inset
          .zip_size(constants.node_inner_size, |p, s| p.maybe_resolve(s)),
        margin: child_style
          .margin
          .resolve_or_zero(constants.node_inner_size.width),
  ...
  for child in flex_items.iter_mut() {
    ...
    child.flex_basis = 'flex_basis: {
      ...
      let flex_basis = child_style
        .flex_basis
        .maybe_resolve(constants.node_inner_size.main(dir));
...
fn resolve_flexible_lengths(
  ...
  let total_original_main_axis_gap =
    sum_axis_gaps(original_gap.main(constants.dir), line.items.len());
  ...
  loop {
    ...
    let (sum_flex_grow, sum_flex_shrink): (f32, f32) = unfrozen
      .iter()
...
fn distribute_remaining_free_space(flex_lines: &mut [FlexLine], constants: &AlgoConstants) {
  for line in flex_lines {
    ...
    if free_space > 0.0 && num_auto_margins > 0 {
      ...
      let justify_content_mode = constants
        .justify_content
  ...
  for line in flex_lines {
    ...
    for child in line.items.iter_mut() {
        ...
        && child.margin_is_auto.cross_end(constants.dir)
      {
        ...
        child.offset_cross =
          align_flex_items_along_cross_axis(child, free_space, max_baseline, constants);
      }
    }
  }
}
...
fn perform_absolute_layout_on_absolute_children(
  ...
  for order in 0..tree.child_count(node) {
    ...
    let margin = child_style
      .margin
      .map(|margin| margin.resolve_to_option(container_width));
    ...
    if let (None, Some(left), Some(right)) = (known_dimensions.width, left, right) {
      let new_width_raw = container_width
    ...
    if let (None, Some(top), Some(bottom)) = (known_dimensions.height, top, bottom) {
      let new_height_raw = container_height
    ...
    let offset_main = if let Some(start) = start_main {
      ...
      match (
        ...
        (JustifyContent::SpaceEvenly, _)
        | (JustifyContent::SpaceAround, _)
...
mod tests {
  ...
  fn correct_constants() {
    let mut tree: Craft3dTree<()> = Craft3dTree::with_capacity(16);
    ...
    let inner_size = Size {
      width: node_size
        .width
        .maybe_sub(padding_border.horizontal_axis_sum()),
      height: node_size
        .height
        .maybe_sub(padding_border.vertical_axis_sum()),
    };
