...
pub fn compute_root_layout(
  ...
  let padding = style
    .padding
    .resolve_or_zero(available_space.width.into_option());
  let border = style
    .border
    .resolve_or_zero(available_space.width.into_option());
...
pub fn round_layout(tree: &mut impl RoundTree, node_id: NodeId) {
  ...
  fn round_content_size(
    ...
    layout.content_size.width =
      round(cumulative_x + unrounded_content_size.width) - round(cumulative_x);
    layout.content_size.height =
      round(cumulative_y + unrounded_content_size.height) - round(cumulative_y);
  }
}
