...
use crate::tree::{
  Cache, Layout, LayoutInput, LayoutOutput, LayoutPartialTree, NodeId, PrintTree, RoundTree,
...
impl core::fmt::Display for Craft3dError {
  fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
    match self {
      Craft3dError::ChildIndexOutOfBounds {
        parent,
        child_index,
        child_count,
      } => {
...
#[cfg(feature = "std")]
impl std::error::Error for Craft3dError {}
...
impl<NodeContext> TraversePartialTree for Craft3dTree<NodeContext> {
...
impl<NodeContext> TraverseTree for Craft3dTree<NodeContext> {}

// PrintTree impl for TaffyTree
impl<NodeContext> PrintTree for Craft3dTree<NodeContext> {
  #[inline(always)]
...
where
  MeasureFunction:
    FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>) -> Size<f32>,
{
  /// A reference to the TaffyTree
  pub(crate) taffy: &'t mut Craft3dTree<NodeContext>,
...
impl<'t, NodeContext, MeasureFunction> TraversePartialTree
  for TaffyView<'t, NodeContext, MeasureFunction>
where
  MeasureFunction:
    FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>) -> Size<f32>,
{
...
impl<'t, NodeContext, MeasureFunction> LayoutPartialTree
  for TaffyView<'t, NodeContext, MeasureFunction>
where
  MeasureFunction:
    FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>) -> Size<f32>,
...
impl<NodeContext> Craft3dTree<NodeContext> {
  ...
  pub fn new_leaf(&mut self, layout: Style) -> Craft3dResult<NodeId> {
  ...
  pub fn new_with_children(&mut self, layout: Style, children: &[NodeId]) -> Craft3dResult<NodeId> {
  ...
  pub fn remove(&mut self, node: NodeId) -> Craft3dResult<NodeId> {
    let key = node.into();
  ...
  pub fn add_child(&mut self, parent: NodeId, child: NodeId) -> Craft3dResult<()> {
  ...
  pub fn insert_child_at_index(
    ...
    if child_index > child_count {
      return Err(Craft3dError::ChildIndexOutOfBounds {
        parent,
        child_index,
        child_count,
      });
  ...
  pub fn set_children(&mut self, parent: NodeId, children: &[NodeId]) -> Craft3dResult<()> {
    let parent_key = parent.into();
  ...
  pub fn remove_child(&mut self, parent: NodeId, child: NodeId) -> Craft3dResult<NodeId> {
  ...
  pub fn child_at_index(&self, parent: NodeId, child_index: usize) -> Craft3dResult<NodeId> {
    let parent_key = parent.into();
    let child_count = self.children[parent_key].len();
    if child_index >= child_count {
      return Err(Craft3dError::ChildIndexOutOfBounds {
  ...
  pub fn children(&self, parent: NodeId) -> Craft3dResult<Vec<NodeId>> {
    Ok(self.children[parent.into()].iter().copied().collect::<_>())
  }

  /// Sets the [`Style`] of the provided `node`
  pub fn set_style(&mut self, node: NodeId, style: Style) -> Craft3dResult<()> {
  ...
  pub fn style(&self, node: NodeId) -> Craft3dResult<&Style> {
    Ok(&self.nodes[node.into()].style)
  }

  /// Return this node layout relative to its parent
  pub fn layout(&self, node: NodeId) -> Craft3dResult<&Layout> {
  ...
  pub fn mark_dirty(&mut self, node: NodeId) -> Craft3dResult<()> {
    /// WARNING: this will stack-overflow if the tree contains a cycle
  ...
  pub fn dirty(&self, node: NodeId) -> Craft3dResult<bool> {
  ...
  pub(crate) fn as_layout_tree(&mut self) -> impl LayoutPartialTree + '_ {
    TaffyView {
      taffy: self,
      measure_function: |_, _, _, _| Size::ZERO,
    }
  }
}
...
mod tests {
  ...
  fn new_should_allocate_default_capacity() {
    ...
    let taffy: Craft3dTree<()> = Craft3dTree::new();
  ...
  fn test_with_capacity() {
    ...
    let taffy: Craft3dTree<()> = Craft3dTree::with_capacity(CAPACITY);
  ...
  #[test]
  fn test_new_leaf() {
    let mut taffy: Craft3dTree<()> = Craft3dTree::new();
  ...
  fn new_leaf_with_context() {
    let mut taffy: Craft3dTree<Size<f32>> = Craft3dTree::new();
  ...
  fn compute_layout_should_produce_valid_result() {
    ...
    let node_result = taffy.new_leaf(Style {
      size: Size {
        width: Dimension::Length(10f32),
        height: Dimension::Length(10f32),
      },
      ..Default::default()
    });
  ...
  fn make_sure_layout_location_is_top_left() {
    ...
    let root = taffy
      .new_with_children(
        Style {
          size: Size {
            width: Dimension::Length(100f32),
            height: Dimension::Length(100f32),
          },
          padding: Rect {
            left: length(10f32),
            right: length(20f32),
